...
//    string equation = "0.5^x-(x+(-2))^2+1";
    ...
    Button button9("/textures/tan.png",{(renderWidth-468),180});
    Button button10("/textures/cos.png",{(renderWidth-355),180});
...
//                    button2.setColor(sf::Color(255, 255, 255));
        ...
        for(double x = lastNegative; x < lastPositive; x += 1/Scaler)                  //draws the graph in increments of 0.1
        {
            ...
            Line[0].position = sf::Vector2f((x*Scaler+CenterX),(ShuntingYard::eval(tree)*Scaler*-1+CenterY));
            ShuntingYard::variables["x"] = x+1/Scaler;
            Line[1].position = sf::Vector2f(((x+1/Scaler)*Scaler+CenterX),(ShuntingYard::eval(tree)*Scaler*-1+CenterY));
            window.draw(Line);
        }
